"use strict";var _createClass=(function(){function defineProperties(target,props){for(var i=0;i < props.length;i++) {var descriptor=props[i];descriptor.enumerable = descriptor.enumerable || false;descriptor.configurable = true;if("value" in descriptor)descriptor.writable = true;Object.defineProperty(target,descriptor.key,descriptor);}}return function(Constructor,protoProps,staticProps){if(protoProps)defineProperties(Constructor.prototype,protoProps);if(staticProps)defineProperties(Constructor,staticProps);return Constructor;};})();function _classCallCheck(instance,Constructor){if(!(instance instanceof Constructor)){throw new TypeError("Cannot call a class as a function");}}(function e(t,n,r){function s(o,u){if(!n[o]){if(!t[o]){var a=typeof require == "function" && require;if(!u && a)return a(o,!0);if(i)return i(o,!0);var f=new Error("Cannot find module '" + o + "'");throw (f.code = "MODULE_NOT_FOUND",f);}var l=n[o] = {exports:{}};t[o][0].call(l.exports,function(e){var n=t[o][1][e];return s(n?n:e);},l,l.exports,e,t,n,r);}return n[o].exports;}var i=typeof require == "function" && require;for(var o=0;o < r.length;o++) s(r[o]);return s;})({1:[function(require,module,exports){var _=require('lodash');var RenderLoop=(function(){function RenderLoop(){var options=arguments.length <= 0 || arguments[0] === undefined?{}:arguments[0];_classCallCheck(this,RenderLoop);this.id = null;this.requested = false;this.ctx = options.canvas.getContext('2d');this.setSize(options.width,options.height);this.fetchShapes = options.fetchShapes;}_createClass(RenderLoop,[{key:"setSize",value:function setSize(w,h){this.width = w;this.height = h;}},{key:"clear",value:function clear(){this.ctx.clearRect(0,0,this.w,this.h);}},{key:"_secretRender",value:function _secretRender(){var _this=this;if(this.requested){this.requested = false;this.clear();this.ctx.save();_.forEach(this.fetchShapes(),function(shape){shape.prerender(_this.ctx);shape.render(_this.ctx);shape.postrender(_this.ctx);});this.ctx.restore();}}},{key:"render",value:function render(){this.requested = true;}},{key:"start",value:function start(){this.id = setInterval(_.bind(this._secretRender,this),33);}},{key:"stop",value:function stop(){clearInterval(this.id);}}]);return RenderLoop;})();module.exports = {RenderLoop:RenderLoop};},{"lodash":4}],2:[function(require,module,exports){var Rectangle=(function(){function Rectangle(){var opts=arguments.length <= 0 || arguments[0] === undefined?{}:arguments[0];_classCallCheck(this,Rectangle);this.x = opts.x;this.y = opts.y;this.width = opts.width;this.height = opts.height;this.fillStyle = opts.fillStyle;}_createClass(Rectangle,[{key:"prerender",value:function prerender(ctx){ctx.save();ctx.translate(this.x,this.y);}},{key:"render",value:function render(ctx){ctx.beginPath();ctx.rect(0,0,this.width,this.height);ctx.closePath();}},{key:"postrender",value:function postrender(ctx){if(this.fillStyle){ctx.fillStyle = this.fillStyle;ctx.fill();}ctx.restore();}}]);return Rectangle;})();module.exports = {Rectangle:Rectangle};},{}],3:[function(require,module,exports){var _=require('lodash');var _require=require('./Shapes');var Rectangle=_require.Rectangle;var _require2=require('./RenderLoop');var RenderLoop=_require2.RenderLoop;var Game=(function(){function Game(){_classCallCheck(this,Game);this.shapes = [];this.addShape(new Rectangle({x:0,y:0,width:500,height:400,fillStyle:'#e5e5e5'}));this.addShape(new Rectangle({x:10,y:10,width:50,height:50,fillStyle:'#ff0055'}));}_createClass(Game,[{key:"getShapes",value:function getShapes(){return this.shapes;}},{key:"addShape",value:function addShape(shape){this.shapes.push(shape);}}]);return Game;})();function startApp(){var canvas=document.getElementById('gamecanvas');var game=new Game();window.myGame = game;var renderLoop=new RenderLoop({canvas:canvas,width:500,height:400,fetchShapes:_.bind(game.getShapes,game)});renderLoop.start();console.log('started!');renderLoop.render();console.log('rendered!');};window.onload = function onAppLoad(){startApp();};},{"./RenderLoop":1,"./Shapes":2,"lodash":4}],4:[function(require,module,exports){(function(global){ /**
 * @license
...
'isKeysFast':isKeysFast, // iterator options
...
return value instanceof Array || toString.call(value) == arrayClass;}; /**
   ...
   */var keys=!nativeKeys?shimKeys:function(object){if(!isObject(object)){return [];}return nativeKeys(object);}; /**
...
var ctor=value.constructor;if(!isFunction(ctor) || ctor instanceof ctor){ // In most environments an object's own properties are iterated before
...
var isObj=isObject(result);if(isObj){var className=toString.call(result);if(!cloneableClasses[className]){return result;}var isArr=isArray(result);} // shallow clone
...
(isArr?forEach:forOwn)(done?result:value,function(objValue,key){result[key] = clone(objValue,deep,callback,undefined,stackA,stackB);});return result;} /**
   ...
   * _.isEmpty('');
   * // => true
   */function isEmpty(value){var result=true;if(!value){return result;}var className=toString.call(value),length=value.length;if(className == arrayClass || className == stringClass || className == argsClass || className == objectClass && typeof length == 'number' && isFunction(value.splice)){return !length;}forOwn(value,function(){return result = false;});return result;} /**
   ...
   * });
   * // => true
   */function isEqual(_x3,_x4,_x5,_x6,_x7,_x8){var _again=true;_function: while(_again) {var a=_x3,b=_x4,callback=_x5,thisArg=_x6,stackA=_x7,stackB=_x8;_again = false; // used to indicate that when comparing objects, `a` has at least the properties of `b`
...
if(a.__wrapped__ || b.__wrapped__){_x3 = a.__wrapped__ || a;_x4 = b.__wrapped__ || b;_x5 = callback;_x6 = thisArg;_x7 = stackA;_x8 = stackB;_again = true;whereIndicator = result = type = otherType = className = otherClass = isArr = undefined;continue _function;} // exit for functions and DOM nodes
if(className != objectClass){return false;} // in older versions of Opera, `arguments` objects have `Array` constructors
var ctorA=a.constructor,ctorB=b.constructor; // non `Object` object instances with different constructors are not equal
...
if(!callback){value = merge(value,source,indicatorObject,callback,stackA,stackB);}}}else {if(callback){result = callback(value,source);if(typeof result == 'undefined'){result = source;}}if(typeof result != 'undefined'){value = result;}}object[key] = value;});}return object;} /**
   ...
   * // => ['moe', 'curly']
   */function at(collection){var index=-1,props=concat.apply(arrayRef,slice(arguments,1)),length=props.length,result=Array(length);while(++index < length) {result[index] = collection[props[index]];}return result;} /**
   ...
   * // => [4, 5, 2, 3, 0, 1]
   */function reduceRight(collection,callback,accumulator,thisArg){var iterable=collection,length=collection?collection.length:0,noaccum=arguments.length < 3;if(typeof length != 'number'){var props=keys(collection);length = props.length;}callback = createCallback(callback,thisArg,4);forEach(collection,function(value,index,collection){index = props?props[--length]:--length;accumulator = noaccum?(noaccum = false,iterable[index]):callback(accumulator,iterable[index],index,collection);});return accumulator;} /**
   ...
   * // => [2, 3, 4]
   */function toArray(collection){if(collection && typeof collection.length == 'number'){return slice(collection);}return values(collection);} /**
   * Examines each element in a `collection`, returning an array of all elements
...
each(['concat','slice','splice'],function(methodName){var func=arrayRef[methodName];lodash.prototype[methodName] = function(){return new lodash(func.apply(this.__wrapped__,arguments));};}); /*--------------------------------------------------------------------------*/ // expose Lo-Dash
...
window._ = lodash;}})(this);}).call(this,typeof global !== "undefined"?global:typeof self !== "undefined"?self:typeof window !== "undefined"?window:{});},{}]},{},[3,1,2]);
